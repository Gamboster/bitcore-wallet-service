...
WalletService.prototype.getBalance = function(opts, cb) {
  ...
  self._getActiveAddresses(function(err, addresses) {
    if (err) return cb(err);
...
WalletService.prototype._decorateTxHistory = function(txs, addresses, proposals, notes, includeExtendedInfo) {
...
WalletService.prototype.getTxHistory = function(opts, cb) {
  ...
  self.storage.fetchAddresses(self.walletId, function(err, addresses) {
    ...
    async.waterfall([
      ...
      var finalTxs = self._decorateTxHistory(res.txs.items, addresses, res.txps, res.notes, opts.includeExtendedInfo);
...
WalletService.prototype._getActiveAddresses = function(cb) {
  ...
  async.parallel([
    ...
    return cb(err, _.uniq(res[0].concat(res[1])), 'address');
  });
};
 ...
 *
 * @param {Object} opts
 * @param {Number} opts.from (defaults to 0)
 * @param {Number} opts.limit
 * @param {Number} opts.includeExtendedInfo[=false] - Include all inputs/outputs for every tx.
 * @returns {TxProposal[]} Transaction proposals, first newer
 */
WalletService.prototype.getTxHistory2 = function(opts, cb) {
  var self = this;
  ...
  self._getActiveAddresses(function(err, addresses) {
    ...
    async.parallel([
      ...
      var finalTxs = self._decorateTxHistory(res[0].items, addresses, res[1], res[2], opts.includeExtendedInfo);
